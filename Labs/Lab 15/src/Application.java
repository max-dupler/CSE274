...
class Graph {
	...
	boolean IsVisited(Vertex myVertex,Vertex[] VisititedVertexes) {
	    for (int i = 0; i < VisititedVertexes.length; i++) {
	        if (VisititedVertexes[i] == myVertex) {
	            return true;
	        }
	    }
	...
	public void depthFirstTraversal(String label) {
    	...
    	while (!myStack.isEmpty()) {
    	    ...
    	    for (int i = 0; i < size; i++) {
    	        Vertex another = VertexArray[i];
    	        
    	        int currIndex = VertexToIndex(CurrentVertex);
    	        int anotherIndex = VertexToIndex(another);
    	        
    	        if (Adjacency[currIndex][anotherIndex] == true) {
    	            if (IsVisited(another, VisititedVertexes))
    	                continue;
    	        
        	        Visit(another);
        	        VisititedVertexes[VisitedCounter] = another;
        	        VisitedCounter++;
        	        
        	        myStack.push(CurrentVertex);
        	        myStack.push(another);
